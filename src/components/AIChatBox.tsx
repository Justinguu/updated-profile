...
export default function AIChatBox({ open, onClose }: AIChatBoxProps) {
  ...
  return (
    <div
      className={cn(
        "fixed bottom-20 right-4 z-50 w-[calc(100%-2rem)] max-w-[440px]",
        open ? "block" : "hidden",
      )}
    >
      <div className="panel flex h-[min(600px,70vh)] flex-col overflow-hidden shadow-2xl">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div>
            <p className="eyebrow">Assistant</p>
            <h2 className="text-sm font-semibold text-foreground">Ask about Justin</h2>
          </div>
          <button onClick={onClose} aria-label="Close chat" className="text-muted-foreground hover:text-foreground transition-colors">
            <XCircle size={20} />
          </button>
        </div>
        <div className="mt-3 h-full overflow-y-auto px-3" ref={scrollRef}>
          ...
          {!error && messages.length === 0 && (
            ...
            <Bot size={36} className="text-primary mb-4" />
            <p className="text-base font-semibold text-foreground mb-1">
              Ask me anything about Justin.
            </p>
            <p className="text-sm text-muted-foreground max-w-[260px]">
              Experience, stack, what he built at Global Med Logistics. Try &ldquo;What has he done with Azure?&rdquo;
            </p>
          </div>
          )}
        </div>
        <form onSubmit={handleSubmit} className="m-3 flex gap-1">
          <button
            type="button"
            className="flex w-10 flex-none items-center justify-center text-muted-foreground hover:text-destructive transition-colors duration-300"
            title="Clear chat"
            onClick={() => setMessages([])}
          >
            <Trash size={24} />
          ...
          <input
            ...
            className="flex-grow rounded-full border border-border bg-background px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
            ref={inputRef}
          />
          <button
            type="submit"
            className="flex w-10 flex-none items-center justify-center text-muted-foreground disabled:opacity-40 hover:text-primary transition-colors duration-300"
...
function ChatMessage({ message: { role, content } }: ChatMessageProps) {
  ...
  return (
    ...
    >
      {isAiMessage && <Bot className="mr-2 flex-none text-primary" size={18} />}
      <div
        className={cn(
          "rounded-lg px-3 py-2 text-sm",
          isAiMessage ? "border border-border bg-background" : "bg-foreground text-background",
        )}
      >
        <ReactMarkdown
          components={{
