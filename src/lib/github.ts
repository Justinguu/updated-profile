...
// Lazy proxy: the client (and the token check) is only created on first use,
// so importing this module during `next build` never throws.
export const githubService: GitHubService = new Proxy({} as GitHubService, {
  get(_target, prop) {
    const instance = GitHubService.getInstance();
